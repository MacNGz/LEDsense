...
%plots time(s) and changes in freq
figure
%plot(time(2:end),changes);
%plots freq
%figure
%plot(time,shorter);
sum(changes)
...
beats=changes>2*average;
figure
plot(time(2:end),beats);
times = find(changes>2*average)/REQ_SAMPLE_RATE;
...
time = time.';
csvwrite(filename,time);


fileID = fopen('ardy3.ino','w');
...
[d2,sr2] = audioread('Handles - Poidles V2.wav');
subplot(312)
specgram(d2(:,1),1024,sr2);
p =size(d2(:,1))
t = linspace(1,5000,p(1));
d2=abs(d2);
figure
plot( t,d2(:,1));
ds=size(d2);
dsiz=ds(1);
dsiz/2205
shorter=decimate(d2(:,1),round(dsiz/2205));
shorts=size(shorter);

t2 = linspace(1,dsiz/sr,shorts(1));
figure
plot( t2,shorter);


changes = diff(shorter);
figure
plot(t2(2:end),changes);
% Read with resampling, casting to mono, and time limits
forcemono = 1;
starttime = 0.5;  % seconds
duration = 1.0;
targetsr = 16000;
[d3,sr2] = audioread('piano.aac',targetsr,forcemono,starttime,duration);
subplot(313)
specgram(d3,512,sr2);
